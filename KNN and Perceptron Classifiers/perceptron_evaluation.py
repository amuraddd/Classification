...
for i, decay_rate in enumerate(decay_grid):
    ...
    a4a_evaluation[decay_rate] = [round(accuracy,2), round(misclassification_rate,2)]


a4a_evaluation_df = pd.DataFrame(a4a_evaluation, index=['Accuracy', 'Misclassification Rate'])
...
rects1 = ax.bar(x - width/2, acc, width, label='Accuracy', color='#8ac4d0')
rects2 = ax.bar(x + width/2, err, width, label='Misclassification Rate')


ax.set_ylabel('Accuracy/Misclassification Rate')
ax.set_xlabel('Decay Rate')
ax.set_title('Accuracy and Misclassification Rate by Stepsize')
ax.set_xticks(x)
...
fig.tight_layout()
fig.savefig('a4a_perceptron_accuracy.png')
plt.show()
...
labels = list(iris_evaluation.keys())
acc = list(iris_evaluation_df.loc['Accuracy'].values)
err = list(iris_evaluation_df.loc['Misclassification Rate'].values)
...
ax.set_ylabel('Accuracy/Misclassification Rate')
ax.set_xlabel('Decay Rate')
ax.set_title(' IRIS Accuracy and Misclassification Rate by Stepsize')
ax.set_xticks(x)
ax.set_xticklabels(labels)
ax.legend()
...
fig.tight_layout()
fig.savefig('iris_perceptron_accuracy.png')
plt.show() 
...
accuracy, misclassification_rate = multi_class_perceptron_accuracy(y_test, predictions, labels=[1,2,3])
print(f'IRIS Accuracy: {round(accuracy*100,2)}%, Misclassification Rate: {round(misclassification_rate*100,2)}%')
